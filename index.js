...
app.post('/api/auth/register', async (req, res) => {
  ...
  if (!user) {
    ...
    const result = await db.prepare('INSERT INTO users (email, username, password_hash) VALUES (?, ?, ?)')
      .run(normalizedEmail, name, passHash);
    user = await db.prepare('SELECT * FROM users WHERE id = ?').get(result.lastInsertRowid);
  } else {
    if (username?.trim()) {
      await db.prepare('UPDATE users SET username = ? WHERE id = ?').run(username.trim(), user.id);
      user.username = username.trim();
    }
    await db.prepare('UPDATE users SET password_hash = ? WHERE id = ?').run(passHash, user.id);
...
app.post('/api/leagues', authMiddleware, async (req, res) => {
  ...
  await db.prepare('INSERT INTO league_members (league_id, user_id, points) VALUES (?, ?, ?)')
...
app.post('/api/leagues/join', authMiddleware, async (req, res) => {
  const { code } = req.body;
  if (!code?.trim()) return res.status(400).json({ error: 'Code requis' });

  const league = await db.prepare('SELECT * FROM leagues WHERE code = ?').get(code.trim().toUpperCase());
  if (!league) return res.status(404).json({ error: 'Ligue introuvable' });

  const existing = await db.prepare('SELECT 1 FROM league_members WHERE league_id = ? AND user_id = ?')
    .get(league.id, req.user.id);
  if (existing) return res.json({ league: await formatLeague(league, req.user.id), alreadyMember: true });
...
app.get('/api/leagues/:id', authMiddleware, async (req, res) => {
  const leagueId = parseInt(req.params.id, 10);
  const member = await db.prepare('SELECT * FROM league_members WHERE league_id = ? AND user_id = ?')
    .get(leagueId, req.user.id);
  ...
  res.json({
    league: await formatLeague(league, req.user.id),
    myPoints: member.points,
    leaderboard,
  });
});

async function formatLeague(league, userId) {
  const row = await db.prepare('SELECT COUNT(*) as c FROM league_members WHERE league_id = ?').get(league.id);
  return {
    id: league.id,
    name: league.name,
    code: league.code,
...
app.get('/api/matches', authMiddleware, async (req, res) => {
  ...
  const enriched = await Promise.all(matches.map(async (m) => ({
    ...m,
    odds: await getOdds.all(m.id),
    myBets: leagueId ? await getMyBets.all(m.id, req.user.id, leagueId) : [],
  })));
...
app.get('/api/status', async (_req, res) => {
  ...
  const bookmakers = (await db.prepare('SELECT DISTINCT bookmaker FROM match_odds ORDER BY bookmaker').all())
    .map((r) => r.bookmaker);
...
async function bootstrap() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    if (!process.env.DATABASE_URL) {
      console.log('⚠️  Pas de DATABASE_URL — comptes effacés à chaque redeploy Render');
    }
